...
import (
	...
	//"github.com/kubernetes-csi/execution-hook/pkg/executionhook"
...
func (ctrl *hookController) hookWorker() {
	workFunc := func() bool {
		...
		if err == nil {
			...
			//newHook, err := ctrl.checkAndUpdateSnapshotClass(hook)
			//if err == nil {
			//	klog.V(5).Infof("passed checkAndUpdateSnapshotClass for snapshot %q", key)
			//	ctrl.updateHook(newHook)
			//}
...
func (ctrl *hookController) hookTemplateWorker() {
	workFunc := func() bool {
		...
		// been add/update/sync
		if err == nil {
			//if ctrl.isDriverMatch(hookTemplate) {
...
func (ctrl *hookController) updateHookTemplate(hookTemplate *crdv1.ExecutionHookTemplate) {
	...
	klog.V(5).Infof("Xing1: updateHookTemplate: %+v", hookTemplate)
	err = ctrl.syncHookTemplate(hookTemplate)
	klog.V(5).Infof("Xing2: updateHookTemplate: %+v", hookTemplate)
	if err != nil {
		if errors.IsConflict(err) {
			// Version conflict error happens quite often and the controller
			// recovers from it easily.
...
func (ctrl *hookController) deleteHook(hook *crdv1.ExecutionHook) {
	...
	/*hookTemplateName := snapshot.Spec.HookTemplateName
	if hookTemplateName == "" {
		klog.V(5).Infof("deleteHook[%q]: hookTemplate not bound", hookKey(hook))
		return
	}
	// sync the hookTemplate when its hook is deleted.  Explicitly sync'ing the
	// hookTemplate here in response to hook deletion prevents the hookTemplate from
	// waiting until the next sync period for its Release.
	klog.V(5).Infof("deleteHook[%q]: scheduling sync of hookTemplate %s", hookKey(hook), hookTemplateName)
	ctrl.hookTemplateQueue.Add(hookTemplateName)*/
}
...
func (ctrl *hookController) deleteHookTemplate(hookTemplate *crdv1.ExecutionHookTemplate) {
	...
	//hookName := hookRefKey(hookTemplate.Spec.ExecutionHookRef)
	//if hookName == "" {
	//	klog.V(5).Infof("deleteHookTemplate[%q]: hookTemplate not bound", hookTemplate.Name)
	//	return
	//}
	// sync the hook when its hookTemplate is deleted.  Explicitly sync'ing the
	// hook here in response to hookTemplate deletion prevents the hook from
	// waiting until the next sync period for its Release.
	//klog.V(5).Infof("deleteHookTemplate[%q]: scheduling sync of hook %s", hookTemplate.Name, hookName)
	//ctrl.hookQueue.Add(hookName)
}
